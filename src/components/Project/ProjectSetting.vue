      ...
      <div class="p-6 space-y-6 bg-white">
        ...
        <div class="space-y-2">
          ...
          <input
            type="text"
            v-model="title"
            class="w-full !h-10 px-3 py-2 input-white"
        ...
        <!-- Presets -->
        <div class="space-y-2">
          <label class="block text-sm font-medium text-gray-700">프로젝트 프리셋</label>
          <div class="grid grid-cols-2 gap-4">
            <div
              v-for="preset in presets"
              :key="preset.name"
              @click="selectPreset(preset)"
              class="p-4 transition-colors border rounded-lg cursor-pointer hover:border-[#4F46E5]"
              :class="{ 'border-[#4F46E5] bg-indigo-50': selectedPreset === preset.name }"
            >
              <div class="mb-2 bg-gray-200 rounded aspect-video"></div>
